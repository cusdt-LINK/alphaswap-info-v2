...
export function getPoolLink(token0Address, token1Address = null, remove = false) {
  if (!token1Address) {
    return (
      `https://swap.alphadao.money/#/` +
      (remove ? `remove` : `add`) +
      `/${
        token0Address?.toLowerCase() === '0xf55af137a98607f7ed2efefa4cd2dfe70e4253b1' ? 'ETH' : token0Address
      }/${'ETH'}`
    )
  } else {
    return (
      ...
      (remove ? `remove` : `add`) +
      `/${token0Address?.toLowerCase() === '0xf55af137a98607f7ed2efefa4cd2dfe70e4253b1' ? 'ETH' : token0Address}/${
        token1Address?.toLowerCase() === '0xf55af137a98607f7ed2efefa4cd2dfe70e4253b1' ? 'ETH' : token1Address
      }`
    )
  }
}
...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://swap.alphadao.money/#/swap?inputCurrency=${token0Address}`
  } else {
    return `https://swap.alphadao.money/#/swap?inputCurrency=${
      token0Address?.toLowerCase() === '0xf55af137a98607f7ed2efefa4cd2dfe70e4253b1' ? 'ETH' : token0Address
    }&outputCurrency=${
      token1Address?.toLowerCase() === '0xf55af137a98607f7ed2efefa4cd2dfe70e4253b1' ? 'ETH' : token1Address
    }`
...
export function getMiningPoolLink(token0Address) {
  return `https://swap.alphadao.money/#/uni/ETH/${token0Address}`
}

export function getUniswapAppLink(linkVariable) {
  let baseUniswapUrl = 'https://swap.alphadao.money'
  if (!linkVariable) {
...
export const urls = {
  showTransaction: (tx) => `https://explorer.kcc.io/tx/${tx}/`,
  showAddress: (address) => `https://explorer.kcc.io/address/${address}/`,
  showToken: (address) => `https://explorer.kcc.io/tokens/${address}/`,
  showBlock: (block) => `https://explorer.kcc.io/blocks/${block}/`,
}
